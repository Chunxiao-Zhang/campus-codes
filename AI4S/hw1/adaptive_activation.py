...
class AdaptiveActivationMLP(nn.Module):
    ...
    def forward(self, xt: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.layers:
            h = torch.tanh(self.alpha * layer(h))
        return self.head(h)


def train_adaptive_activation(
    cfg: TrainConfig,
    ...
    set_seed(cfg.seed)
    model = AdaptiveActivationMLP().to(cfg.device)
    opt = torch.optim.Adam(model.parameters(), lr=cfg.lr)

    history = {"loss": [], "loss_f": [], "loss_ic": [], "loss_bc": [], "rel_l2": []}
    run_name = cfg.case_name if cfg.case_name else "adaptive_activation"
    start = time.time()
